...
public interface SpotifyControllerInterface {
    ...
    @GetMapping("/search")
    public ResponseEntity<?>  querySearch(@RequestHeader("Authorization") String accessToken, @RequestHeader("Refresh-Token") String refreshToken, @RequestParam String query, @RequestParam(defaultValue = "artist,album") String type);
}